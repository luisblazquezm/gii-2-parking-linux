...
*/
            ...
            semwait(ipc.semid, ipc.sem_0 + SEM_MSG_COUNTERS);
                ...
                semsig(ipc.semid, ipc.sem_0 + SEM_MSG);
            ...
            semsig(ipc.semid, ipc.sem_0 + SEM_MSG_COUNTERS);
        }
    ...
    for (i = 0; i < n_chofer; ++i){
        ...
        struct msg_subtipo msgp;
            ...
            while (1){
                ...
                semwait(ipc.semid, ipc.sem_0 + SEM_MSG);
...
int init_ipc(struct ipc_rsrc * ipc, int n_chofer)
{
    ...
    if ((ipc->msgid = create_msg()) == -1){
        perror("init_ipc: create_msh: msgget");
        return -1;
    }
    ...
    if  ((ipc->shmid = create_shmem(ipc->lib_shmsize + ipc->shmsize)) == -1){
        perror("init_ipc: create_shmem: shmget");
        return -1;
    }

    if ((ipc->lib_shmaddr = shmat(ipc->shmid, NULL, 0)) == (char *) -1){
        perror("init_ipc: shmat");
        return -1;
    }
